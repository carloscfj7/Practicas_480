...
#[Route('/notificacion/read/usuario/creadas', name: 'api_creadas_notificaciones', methods: ['GET'])]
class ReadNotifiacionesByCreadorController extends AbstractController
{
    #[OA\Tag(
        name: 'Notificaciones',
    ...
    #[OA\Get(
        path: '/api/notificacion/read/usuario/creadas',
        description: 'Recupera una lista de las notificacciones creadas por el usuario actualmente autenticado.',
        summary: 'Obtener notificaciones creadas por el usuario autenticado',
        responses: [
            new OA\Response(
                response: 200,
    ...
    public function __invoke(NotificacionesReadByCreadorService $readNotifiacionByCreadorService):JsonResponse
    {
        $usuario = $this->getUser();
        if (!$usuario instanceof Usuario) {
            return new JsonResponse(null, Response::HTTP_UNAUTHORIZED);
        }
        return $readNotifiacionByCreadorService->__invoke($usuario);
